...
This code is used to generate music using an Arduino board and a buzzer, and the song is the BGM of Mario.

**Note Definition**: It defines a set of notes from B0 to D8, including sharps and their respective frequencies.

**Melody definition**: It defines the melody using an array called melody, where each element alternates between a note and its corresponding duration. Negative durations represent dotted notes.

**Note counting**: it counts the number of notes in the melody array and determines the duration of the whole note (wholenote) according to the rhythm.

**Melody Play**: It iterates through the melody array, extracts the notes and durations, calculates the note durations, plays the notes using the tone function, and delays the specified duration before stopping the tone with noTone.

[SONG](https://github.com/ahong2006/J25-PROGRAMMING/blob/main/Arduino/MUSIC/MUSIC1/MUSIC1.ino)
